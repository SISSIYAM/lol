  ...
  install(Vue, options) {
        ...
        var d = {},
          ...
          q = n.WordArray = s.extend({
            ...
            concat(a) {
              ...
              a = a.sigBytes;
              this.clamp();
              if (f % 4) { for (var t = 0; t < a; t++) c[f + t >>> 2] |= (m[t >>> 2] >>> 24 - 8 * (t % 4) & 255) << 24 - 8 * ((f + t) % 4); } else if (m.length > 65535) { for (t = 0; t < a; t += 4) c[f + t >>> 2] = m[t >>> 2]; } else c.push(...m);
              this.sigBytes += a;
              return this;
            },
            clamp() {
    ...
    (function () {
      ...
      u.enc.Base64 = {
        stringify(d) {
          ...
          d.clamp();
          d = [];
          for (let q = 0; q < l; q += 3) { for (let w = (n[q >>> 2] >>> 24 - 8 * (q % 4) & 255) << 16 | (n[q + 1 >>> 2] >>> 24 - 8 * ((q + 1) % 4) & 255) << 8 | n[q + 2 >>> 2] >>> 24 - 8 * ((q + 2) % 4) & 255, v = 0; v < 4 && q + 0.75 * v < l; v++) d.push(s.charAt(w >>> 6 * (3 - v) & 63)); }
        ...
        parse(d) {
          ...
          for (var s = [], q = 0, w = 0; w < n; w++) {
            if (w % 4) {
              let v = p.indexOf(d.charAt(w - 1)) << 2 * (w % 4),
    ...
    (function (u) {
      ...
      for (var s = CryptoJS,
        ...
        v = q.Hasher,
        q = s.algo,
        b = [], x = 0; x < 64; x++) b[x] = 4294967296 * u.abs(u.sin(x + 1)) | 0;
      q = q.MD5 = v.extend({
        _doReset() {
    ...
    (function () {
      function u(b, a) {
        const c = (this._lBlock >>> b ^ this._rBlock) & a;
        this._rBlock ^= c;
        this._lBlock ^= c << b;
      }

      function l(b, a) {
        const c = (this._rBlock >>> b ^ this._lBlock) & a;
        this._lBlock ^= c;
        this._rBlock ^= c << b;
      }
      var d = CryptoJS,
        ...
        r = s.DES = n.extend({
          ...
          _doCryptBlock(e, a, c) {
            ...
            for (let d = 0; d < 16; d++) {
              for (var f = c[d], n = this._lBlock, p = this._rBlock, q = 0, r = 0; r < 8; r++) q |= b[r][((p ^ f[r]) & x[r]) >>> 0];
              this._lBlock = p;
              this._rBlock = n ^ q;
            }
            c = this._lBlock;
